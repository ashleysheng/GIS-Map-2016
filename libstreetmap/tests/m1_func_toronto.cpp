...
SUITE(street_queries) {
    ...
    TEST(street_ids_from_name) {
        ...
        expected = {100};
        actual = find_street_ids_from_name("Scholfield Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
            
        expected = {173};
        actual = find_street_ids_from_name("Eglinton Avenue Eastbound");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {63, 246};
        actual = find_street_ids_from_name("Gardiner Expressway Westbound");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {319};
        actual = find_street_ids_from_name("Scollard Street");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {392, 2797, 8718};
        actual = find_street_ids_from_name("Gardiner Expressway West");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {465};
        actual = find_street_ids_from_name("To Highway 401 Eastbound Collectors");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {538};
        actual = find_street_ids_from_name("Charlotte Street");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {611};
        actual = find_street_ids_from_name("Garnock Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {684,688,18598,19044,19045,19048};
        actual = find_street_ids_from_name("Kingston Road");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {757};
        actual = find_street_ids_from_name("Cosburn Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);
        
        expected = {830};
        actual = find_street_ids_from_name("Ivy Avenue");
        std::sort(actual.begin(), actual.end());
        CHECK_EQUAL(expected, actual);

    } //street_ids_from_name test
